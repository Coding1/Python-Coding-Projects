...
class LearningSwitch(DynamicPolicy):
	...
	open_log('learning-switch.log')
        ...
        for switch in fwd_tble:
		for macaddr in fwd_tble[switch]:
			port = fwd_tble[switch][macaddr]
    ...
    def build_policy(self):
		...
		for macaddr in fwd_tble[switch]:
		    if new_policy == None:
		        ...
		        new_policy = (match(switch=int(switch), dstmac=(macaddr)) >>
		                      fwd(fwd_tble[switch][macaddr]))
		    else:
		        new_policy += (match(switch=int(switch), dstmac=(macaddr)) >>
		                       fwd(fwd_tble[switch][macaddr]))
